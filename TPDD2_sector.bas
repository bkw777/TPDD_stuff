10 ' SECTR2.DO 1.5 C.Courson 1989, R.Benson 1989, B.White 2021
...
860 '
870 '
880 '------ initialize variables and open the TPDD and log ports
...
920 ON ERROR GOTO 2830 
930 OPEN "COM:98N1D" FOR OUTPUT AS 1 :OPEN "COM:98N1D" FOR INPUT AS 2 :OPEN P$(P)+":" FOR OUTPUT AS 3
940 '
950 '
...
1110 GOSUB 2480 :PRINT@177,P$(P); :PRINT@293,J$; :PRINT@252,S; :PRINT@212,T;  :PRINT@129,Z$(Z); :PRINT@315,V$(V); 
...
1230 IF X=1 THEN  GOSUB 2520 :GOTO 1890 ' mid-sector, redraw progress screen, fetch next fragment '<-- [SPACE]
...
1320 CLOSE :POKE 62972,201 :CLEAR 50,HIMEM+287 :MAXFILES=1 :MENU  '<-- [ESC]
1330 '
1340 '
1350 '------ select output port
1360 A$="Port  [0]=LCD 1=LPT 2=CRT :" :A=1 :GOSUB 2580 :P=VAL(A$) :IF P<0 OR P>2 THEN  P=0 '<-- [P]
1370 IF P=2 THEN  D=W/5 :SCREEN 1 :WIDTH W :CLS :SCREEN 0
1380 IF P=1 THEN  D=16 :A=0 :GOSUB 2340 :IF A>0 THEN  P=0 :D=8 :GOTO 2690 
1390 IF P=0 THEN  D=8 :CALL 16959
1400 CLOSE 3 :OPEN P$(P)+":" FOR OUTPUT AS 3
1410 GOTO 1220 
1420 '
1430 '
1440 '------ set CRT width
1460 IF W=0 THEN 1220 
1460 A=1 :A$="CRT Width: [1]=40 2=80" :GOSUB 2580 :A=VAL(A$) :IF A<>2 THEN  A=1 '<-- [W]
1470 W=40*A :WIDTH W :D=8*A :GOTO 1220 
1480 '
1490 '
1500 '------ set LPT page length - 0 to disable pagination for continuous dump
1510 A$="LPT page length:" :A=3 :GOSUB 2580 :L=VAL(A$) :GOTO 1010 '<-- [L]
1520 '
1530 '
1540 '------ select track
1550 Y=T :A$="Track :" :A=2 :GOSUB 2580 :T=VAL(A$) :IF T<>Y THEN  S=0 :I=0 :IF X=1 THEN  X=2 '<-- [T]
1560 GOTO 1220 
1570 '
1580 '
1590 '------ toggle sector
...
1640 '
1650 '------ select offset
1660 A=3 :A$="Jump to (000-4F0): " :GOSUB 2580 '<-- [O]
1670 B$=RIGHT$(A$,2) :A$="0"+LEFT$(A$,1) :GOSUB 2390 :B=A*256
1680 A$=B$ :GOSUB 2390 :A=A+B :IF A=0 THEN  J=0 ELSE  IF A>1279 THEN  J=0 ELSE  J=FIX(A/D)
1690 I=J :GOTO 1220 
1700 '
1710 '
...
1860 GOSUB 2520 :TX$=CHR$(48)+CHR$(5)+CHR$(0)+CHR$(0)+CHR$(T)+CHR$(0)+CHR$(S)
1870 GOSUB 2120 :X=1 :IF A=0 THEN  I=0 ELSE 2690 
...
1920 GOSUB 2120 :IF A>0 GOTO 2690 
1930 '------ output the hex pairs
1940 GOSUB 2480 :IF V=1 THEN  PRINT#3,USING"## ";T;:PRINT#3,USING"# ";S;
1950 PRINT#3,J$+" ";:FOR C = 1 TO D :A=ASC(MID$(RX$,C,1)) :GOSUB 2430 :PRINT#3,A$+" "; :NEXT C :IF V=1 THEN 1990 
1960 '------ output the printable bytes
1970 PRINT#3,"  "; :FOR C = 1 TO D :A$=MID$(RX$,C,1) :IF A$<" " OR A$>"~" THEN  PRINT#3,"."; ELSE  PRINT#3,A$;
1980 NEXT C
1990 PRINT#3,"" :I=I+1 :IF I*D > 1279 THEN 2270 ELSE  A$=INKEY$ :IF A$<>"" GOTO 1160 
...
2260 '------ end of sector
2270 I=0
2280 IF Z=0 THEN  SOUND 1180,5 :SOUND 1280,5 :X=0 :R=1 :GOTO 1150 
...
2490 '
2500 '
2510 '------ progress / status header
2520 CALL 16959 :CLS :PRINT "Output:"+P$(P)+"  Track:"T" Sector:"S" "+Z$(Z) :RETURN
2530 '
2540 '
2550 '------ pop-up dialog
2560 ' does not clean up after itself, you must know on returning from hot-key handler
2570 ' whether a pop-up was used and you need to re-draw the screen. R=1 tells you that.
2580 R=1 :B=LEN(A$)+A+2 :C=140-(B/2) :PRINT@C,CHR$(240)+STRING$(B-2,CHR$(241))+CHR$(242);
2590 PRINT@C+40,CHR$(245)+A$+STRING$(A," ")+CHR$(245);
2600 PRINT@C+80,CHR$(246)+STRING$(B-2,CHR$(241))+CHR$(247);
2610 IF A=0 THEN 2650 ELSE  PRINT@C+41+LEN(A$),""; :A$="000" :B=0
2620 B$=INPUT$(1) :IF B$=CHR$(13) THEN 2640 
2630 A$=A$+B$ :B=B+1 :IF B=A THEN 2640 ELSE  PRINT B$; :GOTO 2620 
2640 A$=RIGHT$(A$,A) :RETURN
2650 A$=INKEY$ :IF A$="" THEN 2650 ELSE  RETURN
2660 '
2670 '
2680 '------ display error
2690 SOUND 1280,1 :SOUND 1180,1 :SOUND 1180,1 :SOUND 1280,1 :SOUND 1180,3
2700 ON A GOSUB 2730,2740,2750,,,,2790,2800,2810,2820 
2710 A=0 :GOSUB 2580 :GOTO 1000 
2720 '  "#######################################" max length
2730 A$="Disk I/O error" :RETURN
2740 A$="Insert disk" :RETURN
2750 A$="Drive not responding" :RETURN
2760 '
2770 '
2780 '
2790 A$="Printer off/not connected" :RETURN
2800 A$="Printer not ready" :RETURN
2810 A$="CRT not available" :RETURN
2820 A$="Unexpected Error :"+STR$(B)+" in"+STR$(ERL) :RETURN
2830 IF ERL=84 OR ERL=96 THEN  A=9 :P=2 :RESUME 2690 
2840 A=10 :B=ERR :RESUME 2690 
